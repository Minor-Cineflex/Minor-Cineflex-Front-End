      ...
      <div className="max-w-full max-h-full flex overflow-x-hidden gap-4 custom-scrollbar pb-3 pr-6 snap-x snap-mandatory" 
          ...
          {testData.MinorCineflex.cinema_list[0].cinema_management.movie_list.map((movie, index) => (
              ...
              >
                ...
                <h1 className="text-start text-white text-lg mt-2 font-semibold  truncate">{movie.name}</h1>
              </div>
          ))}
      </div>
      <div className='w-fit h-full flex flex-col justify-center items-center gap-16'>
            <button onClick={() => scroll('right')} className='rotate-180 rounded-2xl'><FaCircleChevronLeft size={40} className='text-[#E7AB79]'/></button>
            <button  onClick={() => scroll('left')} className='rounded-2xl'><FaCircleChevronLeft size={40} className='text-[#E7AB79]'/></button>
      </div>
    </div>
  )
}
